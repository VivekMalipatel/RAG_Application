...
def rasterize_and_encode(page_bytes: bytes, page_num: int) -> tuple[int, str]:
    images = convert_from_bytes(page_bytes, dpi=settings.PDF_IMAGE_DPI)
    img = images[0]
    buf = io.BytesIO()
...
async def convert_to_pdf(file_data: bytes, file_type: str) -> bytes:
    ...
    try:
        ...
        def _convert_pdf():
            ...
            temp_input_path = f"/tmp/{identifier}_input_document"
        ...
        logger.error(f"Error converting file to PDF: {exc}")
        raise ValueError(f"Failed to convert file to PDF: {exc}")
...
async def detect_file_type(file_data: bytes) -> str:
    try:
        ...
        logger.error(f"Error detecting file type: {exc}")
        return ""
...
async def download_file_from_s3_url(s3_url: str) -> bytes:
    try:
        ...
        logger.error(f"Error downloading file from S3 URL {s3_url}: {exc}")
